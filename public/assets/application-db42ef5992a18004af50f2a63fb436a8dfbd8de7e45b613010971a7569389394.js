...
$(document).ready(function(){
	...
	var getLookup = function(){
		...
		//	$('#lookup_errors').show().empty().append("zipcode must be numbers");
			...
			$.getJSON("https://maps.googleapis.com/maps/api/geocode/json?address=" + addy + "," + city + "&components=administrative_area:FL|country:US&key=AIzaSyAdiUvnwghttRAxeemccUDdH6PaGZCxRVA", function(json){
				...
				if (json.results.length > 1){
					...
					console.log("https://maps.googleapis.com/maps/api/geocode/json?address=" + addy + "," + city + "&components=administrative_area:FL|country:US&key=" + google);
				...
				//var API_url = ‘https://openstates.org/graphql 1’;
				var myQuery = "\
					{\
				...
				.then(function(myJson){
					console.log("test: " + JSON.stringify(myJson.data.people.edges[0].node.image));
				 	...
				 	//$.getJSON("https://openstates.org/api/v1/legislators/geo/?lat=" + lat + "&long=" + lng + "&apikey=" + sunlight, function(results) {
				 		...
				 		//var badUrlCheck = results[0].photo_url;

				 		//var goodUrl = badUrlCheck.replace("flhouse","myfloridahouse");

						//$("#headshot").attr("src", goodUrl);
						...
						$('#district_1').val(myJson.data.people.edges[0].node.chamber[0].post.label);
						...
						$("#results_chamber2").empty().append(myJson.data.people.edges[1].node.chamber[0].organization.name);
		 		  		...
		 		  		$('#district_2').val(myJson.data.people.edges[1].node.chamber[0].post.labelt);
						...
						var indexWithEmailInIt = 4;
						console.log("number in aaaray is " + myJson.data.people.edges[0].node.contactDetails.length.toString());
						...
						$.each(myJson.data.people.edges[0].node.contactDetails, function (index, value){
							...
							if (value.type == "email"){
								...
								console.log("email is at index " + index);
